...
使用 **Object.getOwnPropertyDescriptor**[1] 查看，会发现 window.document 的 `configurable` 属性为 false（不可配置的）
...
说到环境中没有 document 对象，`Web Worker` 直呼内行，我曾在**《一文彻底了解Web Worker，十万、百万条数据都是弟弟🔥》**[2]中聊过如何使用 Web Worker，和对应的特性
...
3）Vue 模板表达式的计算是运行在一个沙箱中，模板字符串中的表达式只能获取部分全局对象，详情见**源码**[3]

4）微前端框架 **qiankun**[4] ，为了实现js隔离，在多种场景下均使用了沙箱

## 沙箱的多种实现方式

先聊下 **with**[5] 这个关键字：作用在于改变作用域，可以将某个对象添加到作用域链的顶部
...
：`ctx`作为执行上下文对象，待执行程序`code`可以访问到的变量，必须都来自ctx对象
...
：`ctx`作为执行上下文对象，待执行程序`code`可以访问到的变量，必须都来自ctx对象，如果ctx对象中不存在该变量，直接报错，不再通过作用域链向上查找
...
利用 iframe 来实现一个沙箱是目前最方便、简单、安全的方法，可以把 **iframe.contentWindow**[7] 作为沙箱执行的全局 window 对象
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz/H8M5QJDxMHrLEHKPayEViapq1qKIy6A5lBccpPMsr7ExQPee8WTLoZfSTzxteygbl18eM4B6StA6ZWK9dN886gg/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)document.png
